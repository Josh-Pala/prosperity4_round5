...
# H14: per-symbol EMA fair value taker for UV_VISOR
UV_VISOR_SYMS = [
    "UV_VISOR_AMBER", "UV_VISOR_MAGENTA", "UV_VISOR_ORANGE",
    "UV_VISOR_RED", "UV_VISOR_YELLOW",
]
UV_EMA_ALPHA = 0.005     # ~ half-life ~140 ticks
UV_EMA_K = 6             # take when mid deviates by >= K from EMA
UV_TAKE_SIZE = 3
...
class Trader:
    def run(self, state: TradingState):
        ...
        # ---- UV_VISOR EMA mean-reversion taker (H14) ----
        ema_store = store.setdefault("uv_ema", {})
        for s in UV_VISOR_SYMS:
            if s in engaged_pair_legs:
                continue
            ...
            if m is None:
                continue
            ema_prev = ema_store.get(s)
            if ema_prev is None:
                ema_store[s] = m
                continue
            ema_new = ema_prev + UV_EMA_ALPHA * (m - ema_prev)
            ema_store[s] = ema_new
            bb, ba = best_levels(d)
            ...
            extra: List[Order] = []
            # mid significantly above EMA — sell at bid
            if m - ema_new > UV_EMA_K:
                cap = LIMIT + pos
                for o in existing:
                    if o.quantity < 0:
                        cap -= -o.quantity
                qty = min(UV_TAKE_SIZE, max(0, cap))
                if qty > 0:
                    extra.append(Order(s, bb, -qty))
            elif ema_new - m > UV_EMA_K:
                cap = LIMIT - pos
                for o in existing:
                    if o.quantity > 0:
                        cap -= o.quantity
                qty = min(UV_TAKE_SIZE, max(0, cap))
                if qty > 0:
                    extra.append(Order(s, ba, qty))
            if extra:
                result[s] = existing + extra
