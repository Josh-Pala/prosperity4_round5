"""
glauco_v1_trader — full universe with PEBBLES (v11) + ROBOT (H25)
+ TRANSLATOR (T42 optimized).

Built on top of mm_pairs_full_v3_1 with TRANSLATOR upgrade:
  - Pair set: 5 pairs (was 2), top-3 EDA pairs added
    + ASTRO_BLACK | VOID_BLUE        sum     z=1.2  (NEW, EDA rank 1)
    + ECLIPSE_CHARCOAL | VOID_BLUE   spread  z=1.2  (NEW, EDA rank 2)
    + ECLIPSE_CHARCOAL | SPACE_GRAY  sum     z=1.2  (NEW, EDA rank 5)
  - Execution: aggressive (taker) — beats passive by +11k for this family
  - EXIT_Z: 0.2 for TRANSLATOR (was 0.3) — more cycles
  - ECLIPSE_CHARCOAL removed from MM_UNIVERSE (now in pairs)

Per-family routing:
  - PASSIVE_ENTRY_FAMILIES: MICROCHIP, SNACKPACK, SLEEP_POD (was +TRANSLATOR)
  - EXIT_Z_BY_FAMILY: TRANSLATOR=0.2, others=0.3 default

Standalone TRANSLATOR backtest: 99,916 (vs 50,696 baseline = +97.1%).
Expected total: ~718k (vs 685k v3.1, vs 562k mm_pairs_full).
"""
...
PAIRS: List[Tuple[str, str, str, float]] = [
    ...
    # UV_VISOR — H5 combo: 4 pair + aggressive MM
    ("UV_VISOR_ORANGE", "UV_VISOR_RED", "spread", 1.8),
    ("UV_VISOR_MAGENTA", "UV_VISOR_YELLOW", "sum", 1.2),
    ("UV_VISOR_AMBER", "UV_VISOR_RED", "sum", 1.2),
...
# H5 combo: per-symbol MM quote size + skew (UV_VISOR)
MM_QUOTE_SIZE_BY_SYM: Dict[str, int] = {
    "UV_VISOR_AMBER": 8,
    "UV_VISOR_MAGENTA": 8,
    "UV_VISOR_ORANGE": 8,
    "UV_VISOR_RED": 8,
...
MM_SKEW_BY_SYM: Dict[str, float] = {
    "UV_VISOR_AMBER": 0.15,
    "UV_VISOR_MAGENTA": 0.15,
    "UV_VISOR_ORANGE": 0.15,
    "UV_VISOR_RED": 0.15,
    "UV_VISOR_YELLOW": 0.15,
}
...
class Trader:
    def run(self, state: TradingState):
        ...
        for sym in MM_UNIVERSE:
            ...
            skew_per = MM_SKEW_BY_SYM.get(sym, MM_SKEW_PER_UNIT)
            skew = int(round(position * skew_per))
            ...
            qsize = MM_QUOTE_SIZE_BY_SYM.get(sym, MM_QUOTE_SIZE)
            buy_qty = min(qsize, buy_capacity)
            sell_qty = min(qsize, sell_capacity)
