...
class Trader:
    def run(self, state: TradingState):
        ...
        for sym in MM_UNIVERSE:
            ...
            # H8: UV_VISOR — add second deeper layer at bb+3/ba-3 with smaller size
            if sym.startswith("UV_VISOR_") and spread >= 6:
                deep_bid = best_bid + 3
                deep_ask = best_ask - 3
                if deep_bid < deep_ask:
                    deep_buy = min(3, max(0, LIMIT - position - buy_qty))
                    deep_sell = min(3, max(0, LIMIT + position - sell_qty))
                    if deep_buy > 0:
                        mm_orders.append(Order(sym, int(deep_bid), int(deep_buy)))
                    if deep_sell > 0:
                        mm_orders.append(Order(sym, int(deep_ask), -int(deep_sell)))
            if mm_orders:
