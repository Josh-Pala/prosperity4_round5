"""
glauco_v5_trader — v1 with PANEL_1X2 REMOVED from MM.

PANEL_1X2 bleeds ~9k each day in the MM layer (day 2: -9595, day 3: -9030,
day 4: -126). Deep EDA showed:
  - Biggest spread (~12 ticks) but also strongest intraday drift.
  - 2500-tick chunks see drift up to -800/+582 → MM accumulates adverse inventory.
  - Tested in v3 with full MM block + skew → still negative.
  - Tested in v4 pair tuning alone → unchanged on PANEL_1X2.
  - Decision: simply DON'T quote PANEL_1X2.

All other v1 logic preserved.

ORIGINAL v1 doc:
  full universe with PEBBLES (v11) + ROBOT (H25) + TRANSLATOR (T42 optimized).
...
MM_UNIVERSE: List[str] = [
    ...
    "MICROCHIP_SQUARE",
    # PANEL_1X2 removed: bleeds ~9k/day in MM (drift > MM edge)
    "UV_VISOR_AMBER",
    "SLEEP_POD_SUEDE",
    "SLEEP_POD_LAMB_WOOL",
