...
class Trader:
    def run(self, state: TradingState):
        ...
        for sym in MM_UNIVERSE:
            ...
            # OXY: full-LIMIT capacity per side (deep book absorbs it)
            mm_size = LIMIT if sym.startswith("OXYGEN_SHAKE_") else MM_QUOTE_SIZE
            buy_qty = min(mm_size, buy_capacity)
            sell_qty = min(mm_size, sell_capacity)
