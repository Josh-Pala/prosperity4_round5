...
class Trader:
    def run(self, state: TradingState):
        ...
        for sym in MM_UNIVERSE:
            ...
            # OXY: imbalance-driven micro-skew (alpha ~0.05 corr w/ next-tick mid;
            # eda5/oxygen_shake/ confirms predictive power)
            if sym.startswith("OXYGEN_SHAKE_"):
                ...
                if tot > 0:
                    imb = (bv - av) / tot
                    if imb > 0.6:
                        # Bid pressure → expect mid up: don't sell cheap, raise both
                        ask_px += 1
                    elif imb < -0.6:
                        # Ask pressure → expect mid down: don't buy expensive, lower both
                        bid_px -= 1
            if bid_px >= best_ask:
                bid_px = best_ask - 1
            if ask_px <= best_bid:
