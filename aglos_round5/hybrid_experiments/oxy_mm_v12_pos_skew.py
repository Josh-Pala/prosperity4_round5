...
class Trader:
    def run(self, state: TradingState):
        ...
        for sym in MM_UNIVERSE:
            ...
            # OXY: position-aware skew. Spread is 12-15 ticks → we can step
            # both quotes 1-2 ticks toward closing inventory.
            if sym.startswith("OXYGEN_SHAKE_") and abs(position) >= 4:
                step = 1 if abs(position) < 7 else 2
                if position > 0:
                    bid_px -= step
                    ask_px -= step
                else:
