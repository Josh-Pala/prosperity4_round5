...
class Trader:
    def run(self, state: TradingState):
        ...
        for sym in MM_UNIVERSE:
            ...
            # OXY: join-best on OXYGEN_SHAKE — sim shows +7.5k vs improve+1
            ...
            # OXY dual-layer: top tier at join (bid_px=bb, ask_px=ba), then
            # add a second tier 1 tick worse (bb-1, ba+1) for the next 5 lots
            # of capacity. Fills there are unlikely (no trades go past BBO)
            # but they're "free" if the queue at L1 saturates. Cost: nothing,
            # since they only fill if BBO is broken which means we'd want them.
            if sym.startswith("OXYGEN_SHAKE_"):
                extra_buy = max(0, LIMIT - position - buy_qty)
                extra_sell = max(0, LIMIT + position - sell_qty)
                if extra_buy > 0:
                    mm_orders.append(Order(sym, int(bid_px) - 1, int(min(5, extra_buy))))
                if extra_sell > 0:
                    mm_orders.append(Order(sym, int(ask_px) + 1, -int(min(5, extra_sell))))
