...
class Trader:
    def run(self, state: TradingState):
        ...
        for sym in MM_UNIVERSE:
            ...
            # OXY layered ladder: spread is ~12-15 ticks, deep book — add a
            # join-best tier for extra capacity (eda5/oxygen_shake/mm_strategy_sim.py
            # showed v9 layered ~+19k vs single-level baseline)
            if sym.startswith("OXYGEN_SHAKE_") and spread >= 8:
                extra_buy = max(0, LIMIT - position - buy_qty)
                extra_sell = max(0, LIMIT + position - sell_qty)
                join_buy = min(3, extra_buy)
                join_sell = min(3, extra_sell)
                if join_buy > 0 and best_bid < int(bid_px):
                    mm_orders.append(Order(sym, int(best_bid), int(join_buy)))
                if join_sell > 0 and best_ask > int(ask_px):
                    mm_orders.append(Order(sym, int(best_ask), -int(join_sell)))
