"""
v06 — Time-multiplexed: only one mode is active at a time on XS/S.
  - When pair signal is engaged (cur != 0): pair takes ±10 on XS/S, MM works on M/L/XL only.
  - When pair is flat (cur == 0): MM operates on all 5 legs (full constant-sum freedom).
"""
...
MM_FALLBACK_LEGS = ["PEBBLES_M", "PEBBLES_L", "PEBBLES_XL"]
...
class Trader:
    def run(self, state: TradingState):
        ...
        if cur != 0:
            # PAIR active: pair on XS/S, MM on M/L/XL
            for sym, tgt in [(a, PAIR_SIZE * cur), (b, -PAIR_SIZE * cur)]:
            ...
            mm_legs = MM_FALLBACK_LEGS
        else:
            # PAIR flat: MM on all 5 legs
            mm_legs = ALL_PEBBLES
