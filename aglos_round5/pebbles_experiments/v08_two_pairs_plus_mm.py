"""
v08 — Two pairs + MM:
  - Pair P1: PEBBLES_XS | PEBBLES_S (spread)  entry_z=2.0  (the proven winner)
  - Pair P2: PEBBLES_S  | PEBBLES_M (sum)     entry_z=2.0  (most stationary, adf=0.054)
  - When BOTH pairs are flat → MM on all 5 legs.
  - Pair targets are ADDED. Caps still ±10 per leg.

Rationale: P2 brings M into pair-mode. Together they consume up to 5 legs (XS, S, S, M)
overlap on S. Risk: targets on S can sum to ±20 → clamp.
"""
...
class Trader:
    def run(self, state: TradingState):
        ...
        # Apply pair leg orders (only legs with non-zero target)
        engaged_legs = set()
        ...
        # MM only on legs not engaged by pairs
