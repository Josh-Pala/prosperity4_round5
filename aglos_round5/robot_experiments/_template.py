"""ROBOT-only template — pair-trading on ROBOT family.

A single experiment is parameterised by PAIRS list (a,b,sign,entry_z) and
EXIT_Z / SIZE / WARMUP. No MM layer (the EDA shows no constant-sum invariant,
so MM-with-fair is not applicable; using passive MM on top would only add
generic spread capture and is left for a later layer).

The actual experiment files override PAIRS at the top.
"""
...
# === EXPERIMENT-SPECIFIC PARAMETERS (override per file) ===
