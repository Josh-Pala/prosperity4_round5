"""H1 EWMA, alpha=0.001 (half-life ~700)."""
...
class Trader:
    def run(self, state: TradingState):
        ...
        for a, b, sign, entry_z in PAIRS:
            ...
            st = stats.setdefault(key, {"n": 0, "mean": 0.0, "var": 0.0})
            st["n"] += 1
            n = st["n"]
            if n == 1:
                st["mean"] = sig
                ...
                # EWMA mean and variance (West 1979)
                prev_mean = st["mean"]
                st["mean"] = (1 - ALPHA) * prev_mean + ALPHA * sig
                st["var"] = (1 - ALPHA) * (st["var"] + ALPHA * (sig - prev_mean) ** 2)
            if n < WARMUP:
