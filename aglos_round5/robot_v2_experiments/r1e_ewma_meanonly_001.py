"""H1' EWMA mean only (Welford std). alpha=0.001."""
...
class Trader:
    def run(self, state: TradingState):
        ...
        for a, b, sign, entry_z in PAIRS:
            ...
            st = stats.setdefault(key, {"n": 0, "ewma_mean": 0.0, "M2": 0.0, "global_mean": 0.0})
            st["n"] += 1
            n = st["n"]
            # EWMA mean
            if n == 1:
                ...
                st["ewma_mean"] = (1 - ALPHA) * st["ewma_mean"] + ALPHA * sig
                # Welford for variance, anchored on global mean
                prev_gm = st["global_mean"]
                st["global_mean"] += (sig - prev_gm) / n
                st["M2"] += (sig - prev_gm) * (sig - st["global_mean"])
            if n < WARMUP:
                targets[key] = 0; continue
            var = st["M2"] / (n - 1) if n > 1 else 0.0
            sd = var ** 0.5
            if sd <= 1e-9: continue
            z = (sig - st["ewma_mean"]) / sd
