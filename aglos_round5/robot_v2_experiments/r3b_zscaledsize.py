"""H3'' z-scaled SIZE: pair contributes more units when |z| is large.
size = clip(SIZE_BASE * (|z| / entry_z), MIN, MAX). The leg cap will
auto-clamp at LIMIT=10."""
...
class Trader:
    def run(self, state: TradingState):
        ...
        for a, b, sign, entry_z in PAIRS:
            ...
            targets[key] = cur
            if cur != 0:
                # Scale size by |z| / entry_z
                size = int(SIZE_BASE * abs(z) / entry_z)
