"""
SLEEP_POD pair scanner V2 — uses the avg-cost simulator with proper
position-limit clamping and mark-to-market at end (matches what FINAL_GLAUCO
actually realises during a backtest).

For each (a, b, form, entry_z) it returns true PnL across day 2/3/4 plus
per-day breakdown. Forms tested:
  - spread     (a - b)
  - sum        (a + b)
  - ols_spread (a - beta*b, beta from full-sample OLS)
  - log_ratio  (log(a) - log(b))  — multiplicative; only useful if PnL > 0

Top candidates are then evaluated for cross-pair correlation and added to a
combined sim to check incremental PnL on top of the baseline.
"""
...
def simulate_pair(mids: pd.DataFrame, days: pd.Series,
                  a: str, b: str, form: str, entry_z: float,
                  beta: float = 1.0):
    """
    Single-pair simulator with:
      - running mean/M2 z-score (matches FINAL_GLAUCO)
      - leg target = SIZE * pos for a, +/- SIZE * pos for b (spread/sum)
      - for ols_spread: leg target on b is scaled by beta but rounded to int
        for execution; we approximate with -beta*SIZE rounded.
      - position limit clamp at +/- LIMIT
      - avg-cost basis, realized PnL on closing/flipping, MTM at last tick
    Returns (total_pnl, n_trades, per_day_pnl_dict).
    """
    if form == "spread":
        sig = mids[a] - mids[b]
        leg_b_mult = -1.0
    ...
    total = leg_realized[a] + leg_realized[b]
    return total, n_trades, by_day


def main():
    print("Loading mids...")
    mids, days = load_mids()
    print(f"Loaded {len(mids)} ticks across days {sorted(days.unique())}")
    ...
    df.to_csv(OUT_DIR / "pair_scan_v2_results.csv", index=False)
    ...
    print("\n=== Robust: positive on >= 2 of 3 days, top 15 by PnL ===")
    robust = df[df["pos_days"] >= 2].sort_values("total_pnl", ascending=False)
    print(robust.head(15)[cols].to_string(index=False))

    print("\n=== Fully robust: positive on all 3 days, top 10 by PnL ===")
    full = df[df["pos_days"] == 3].sort_values("total_pnl", ascending=False)
    print(full.head(10)[cols].to_string(index=False))

    print("\n=== Baseline (POLYESTER-COTTON spread, all entry_z) ===")
    base = df[(df["a"] == "SLEEP_POD_COTTON") &
              (df["b"] == "SLEEP_POD_POLYESTER") &
              (df["form"] == "spread")].sort_values("entry_z")
    print(base[cols].to_string(index=False))

    print(f"\nSaved {len(df)} combos to {OUT_DIR/'pair_scan_v2_results.csv'}")
